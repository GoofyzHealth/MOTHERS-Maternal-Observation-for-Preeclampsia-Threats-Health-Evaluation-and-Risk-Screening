...
with st.sidebar:
    with st.sidebar:
        tabs = on_hover_tabs(tabName=['Dashboard', 'Deteksi Dini', 'Database'], 
                             ...
                             styles = {'navtab': {'background-color':'#6431F7',
                                                  'color': '#FFFFFF',
                                                  'font-size': '18px',
                                                  'transition': '.2s',
                                                  'white-space': 'nowrap',
...
if tabs == 'Deteksi Dini':
    ...
    # Judul Halaman
    st.markdown('<h1 style="color:#6431F7;">Deteksi Dini Risiko Preeklamsia</h1>', unsafe_allow_html=True)
    ...
    if st.button('**Klasifikasi Level Risiko** :heart:' ):
        ...
        if risiko == 'Anda **tidak berisiko** untuk terkena preeklamsia':
            risiko_pe = "Tidak Berisiko"
...
if tabs == 'Database':
    ...
    if authentication_status:
        ...
        col_hello, col_logout = st.columns([5, 1])

        with col_hello:
            st.markdown(
                f"<h2 style='margin-bottom:0;'>Halo, <strong>{name}</strong></h2>",
                unsafe_allow_html=True
            )

        with col_logout:
            st.markdown("<div class='logout-align'>", unsafe_allow_html=True)
